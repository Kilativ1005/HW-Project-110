...
def get_random_question_answer(conn):
    ...
    # Move lines from while loop here
    random_index = random.randint(0,len(questions) - 1)
    random_question = questions[random_index]
    conn.send(random_question.encode('utf-8'))
    # Get answer using random_index from answers list and store in random_answers list    
    random_answer = answers[random_index]
    # return index, question and answers
    ...
    while True:
        try:
            ...
            if message:
                ...
                # Remove the question by calling remove_question(index) method 
                remove_question(index)
                # Get new question using get_random_question_answer(conn) method and update index, question, answer variables
                index, question, answer = get_random_question_answer(conn)
...
while True:
    ...
    # Move the code to select the random question in get_random_question_answer function
    
  
    
    # Create a newThread which target clientThread function and send conn as argument
